...
toolchain_resolver="${script_dir}/cpkt-toolchains.sh"
tmp_root=""
toolchain_cc=""
toolchain_ar=""
...
assert_elf_rpath_rejected() {
  ...
  [[ -x "${toolchain_cc}" ]] || fail "pinned Bootlin compiler is required for the ELF RPATH fixture"
  [[ -x "${toolchain_ar}" ]] || fail "pinned Bootlin archiver is required for the ELF RPATH fixture"
...
EOF
  "${toolchain_cc}" -fPIC -c "${fixture_root}/pid0.c" -o "${fixture_root}/pid0.o"
  "${toolchain_ar}" rcs "${stage_root}/lib/libpid0.a" "${fixture_root}/pid0.o"
  "${toolchain_cc}" -shared "${fixture_root}/pid0.o" -Wl,-soname,libpid0.so.0 -Wl,-rpath,/opt/libpid0-fixture \
...
assert_elf_interpreter_rejected() {
  local fixture_root="${tmp_root}/interpreter-fixture"
  local fixture_dist="${fixture_root}/dist"
  local stage_root="${fixture_root}/stage/libpid0-0.0.0-x86_64-linux-gnu"
  local output_path="${fixture_root}/verify.log"

  mkdir -p "${stage_root}/bin" \
    "${stage_root}/include/pid0" \
    "${stage_root}/lib/cmake/pid0" \
    "${stage_root}/lib/pkgconfig" \
    "${stage_root}/share/doc/libpid0" \
    "${fixture_dist}"
  printf '%s\n' 'int pid0_fixture_symbol(void) { return 0; }' > "${fixture_root}/pid0.c"
  printf '%s\n' 'int pid0_fixture_symbol(void);' > "${stage_root}/include/pid0/pid0.h"
  printf '%s\n' 'int main(void) { return 0; }' > "${fixture_root}/main.c"
  "${toolchain_cc}" -fPIC -c "${fixture_root}/pid0.c" -o "${fixture_root}/pid0.o"
  "${toolchain_ar}" rcs "${stage_root}/lib/libpid0.a" "${fixture_root}/pid0.o"
  "${toolchain_cc}" -shared "${fixture_root}/pid0.o" -Wl,-soname,libpid0.so.0 \
    -o "${stage_root}/lib/libpid0.so.0.0.0"
  ln -s libpid0.so.0.0.0 "${stage_root}/lib/libpid0.so.0"
  ln -s libpid0.so.0 "${stage_root}/lib/libpid0.so"
  printf '%s\n' 'include(CMakeFindDependencyMacro)' > "${stage_root}/lib/cmake/pid0/pid0Config.cmake"
  ...
  printf '%s\n' 'fixture license' > "${stage_root}/share/doc/libpid0/LICENSE"
  printf '%s\n' 'fixture readme' > "${stage_root}/share/doc/libpid0/README.md"
  "${toolchain_cc}" "${fixture_root}/main.c" \
    -Wl,--dynamic-linker,/opt/libpid0-fixture-loader \
    -o "${stage_root}/bin/pid0-fixture"
  (
    cd "${fixture_root}/stage"
    tar --sort=name --owner=0 --group=0 --numeric-owner -czf \
      "${fixture_dist}/libpid0-0.0.0-x86_64-linux-gnu.tar.gz" \
  ...
  if PID0_DIST_DIR="${fixture_dist}" "${package_verify}" > "${output_path}" 2>&1; then
    fail "non-portable ELF interpreter was accepted in a release fixture"
  fi
  grep -F 'contains non-portable ELF interpreter' "${output_path}" >/dev/null ||
    fail "ELF interpreter rejection did not report non-portable runtime metadata"
}
...
main() {
  ...
  [[ -x "${toolchain_resolver}" ]] || fail "missing lifecycle toolchain resolver: ${toolchain_resolver}"
  mkdir -p "${repo_root}/build"
  tmp_root="$(mktemp -d "${repo_root}/build/package-privacy.XXXXXX")"
  toolchain_description="$("${toolchain_resolver}" ensure x86_64-linux-gnu)"
  toolchain_cc="$(sed -n 's/^cc=//p' <<<"${toolchain_description}" | tail -1)"
  toolchain_ar="$(sed -n 's/^ar=//p' <<<"${toolchain_description}" | tail -1)"
